...
for idx, model_path in enumerate(model_paths):
    ...
    for day in date_range.day.unique():
        ...
        if model == "OBS":
            hourly_files = list(
                (model_path / "tb_pcp").glob("merg_*" + date_str + "*nc")
            )
            assert len(hourly_files) == 24
            tbb = xr.open_mfdataset(hourly_files).Tb
        else:
            hourly_files = list(
                (model_path / "olr_pcp_instantaneous").glob(
                    "pr_rlut_*" + date_str + "*nc"
                )
            )
            assert len(hourly_files) == 24
        ...
        Features = tobac.feature_detection_multithreshold(
            tbb_iris, dxy, **parameters_features
        )
        Features.to_xarray().to_netcdf(
            savedir / ("Features_" + model + "_" + date_str + ".nc")
        )

        # Perform segmentation and save results
        print("Starting segmentation for ", date_str)
        Mask, Features_seg = tobac.segmentation.segmentation(
            Features, tbb_iris, dxy, **parameters_segmentation
        )
        xr.DataArray.from_iris(Mask).to_netcdf(
            savedir / ("Mask_" + model + "_" + date_str + ".nc")
        )
        Features_seg.to_xarray().to_netcdf(
            savedir / ("Features_seg_" + model + "_" + date_str + ".nc")
        )
