...
tbb_xr = xr.open_dataset('/glade/scratch/kukulies/MCSMIP/DYAMOND/Winter/OBS/tb_pcp/merg_2020022823_4km-pixel.nc').Tb
tbb = tbb_xr.to_iris()
...
for model in models:
    ...
    for day in date_range.day.unique():
        ...
        print(date_str)
        daily_file = list(data_dir.glob("Features_" +model+"_" + date_str + ".nc"))[0]
    ...
    all_features = utils.combine_feature_dataframes(dataframe_list, old_feature_column_name= 'daily_features')

    #### perform linking of features ####
    logging.info('Starting trajectory linking for ', model, 'from ', str(start), 'to', str(end))
    tracks=linking_trackpy(all_features, tbb, dt=dt,dxy=dxy,**parameters_linking)
    tracks.to_xarray().to_netcdf(data_dir / ('Tracks_' + model + '_'+ str(start)[0:10] + '-'+ str(end)[0:10] + '.nc'))
